...
from dbexport.model import Review, Product
...
for product, review_count, avg_rating in session.query(
        Product,
        reviews_statments.c.review_count,
        reviews_statments.c.avg_rating
).outerjoin(reviews_statments, Product.id == reviews_statments.c.product_id):
        csv_writer.writerow({
            "name": product.name,
            "level": product.level,
